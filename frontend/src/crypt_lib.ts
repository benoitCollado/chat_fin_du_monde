...
function hammingDecodeCodeword(cw: number): number {
...
export function eccEncode(bytes: Uint8Array): Uint8Array {
...
export function eccDecode(bytes: Uint8Array): Uint8Array {
  if (bytes.length % 2 !== 0) throw new Error("Longueur ECC invalide");
  const out = new Uint8Array(bytes.length / 2);
  let j = 0;
...
export interface Identity {
  signPub: Uint8Array;  // Ed25519 pub
  signPriv: Uint8Array; // Ed25519 priv
  dhPub: Uint8Array;    // X25519 pub
  dhPriv: Uint8Array;   // X25519 priv
}

export interface WirePayload {
  eph_pub: string; // base64url
  sig: string;     // base64url
  nonce: string;   // base64url
  aad?: string | undefined;    // base64url
  ct: string;      // base64url  (APRES ECC)
}
 ...
 *  Dérivation & AEAD
 *  ========================= */
function deriveSessionKey(shared: Uint8Array, salt: Uint8Array): Uint8Array {
...
export async function encryptForRecipient(
  ...
  recipientDhPub_b64u: string,
  senderSignPriv_b64u: string,
  aad?: Uint8Array
): Promise<WirePayload> {
  await sodium.ready;
...
export async function decryptFromSender(
  payload: WirePayload,
  senderSignPub_b64u: string,
  myDhPriv_b64u: string
): Promise<Uint8Array> {
  await sodium.ready;
  ...
  if (!ok) throw new Error("Signature invalide de la clé éphémère.");
