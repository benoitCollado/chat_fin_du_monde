...
import { b64u, generateIdentity, encryptForRecipient, decryptFromSender } from "./crypt_lib.js";
let A = null; // Alice identity (serialized)
let B = null; // Bob   identity (serialized)
...
(async () => {
    ...
    document.getElementById("send").onclick = async () => {
        if (!A || !B) {
            alert("Génère d'abord Alice et Bob");
            return;
        }
        const msgStr = document.getElementById("msg").value || "Bonjour Bob (avec ECC)!";
